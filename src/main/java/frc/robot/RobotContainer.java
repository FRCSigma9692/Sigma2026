...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        // NamedCommands.registerCommand("StartIntake",Commands.run(()->
        // intake.runIntake(0.5)));
        // NamedCommands.registerCommand("StopIntake", Commands.run(()->
        // intake.StopIntake()));
        NamedCommands.registerCommand("FeederTransfer", (Commands.run(() -> feeder.FeederNoPID(FeederRPM)))
                .alongWith(Commands.run(() -> transfer.runShooterRPM(FeederRPM))));
        NamedCommands.registerCommand("FeederTransferOff",
                (Commands.run(() -> feeder.Stop())).alongWith(Commands.run(() -> transfer.stopShooter())));
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                ...
                drivetrain.applyRequest(() -> drive.withVelocityX(-User1.getLeftY() * MaxSpeed * speed) // Drive forward
                                                                                                        // with negative
                                                                                                        // Y (forward)
                        .withVelocityY(-User1.getLeftX() * MaxSpeed * speed) // Drive left with negative X (left)
                        .withRotationalRate(-User1.getRightX() * MaxAngularRate * speed) // Drive counterclockwise with
                                                                                         // negative X (left)
                ));

...
Pose2d ShooterPose =
    ...
    for (int i = 0; i< 20; i++){
    double OffsetX = ShooterVelX*tofmap.get(lookAheadDist);
    double OffsetY = ShooterVelY * tofmap.get(lookAheadDist);
    ...
    double TOFFinal = tofmap.get(lookAheadDist);
    ...
    FlyWheelSpeed = speedmap.get(compenTranslation2d.getDistance(new Translation2d(commandSwerveDrivetrain.HubX, commandSwerveDrivetrain.HubY)));
